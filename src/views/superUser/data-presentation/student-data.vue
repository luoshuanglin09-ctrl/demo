        ...
        <div class="search">
            <div class="search-box">
                <div class="search-space">
                    <el-select v-model="opgrade" placeholder="选择" style="width: 240px">
                        <el-option v-for="item in grade" :key="item.gradeclass" :label="item.gradeclass"
                            :value="item.gradeclass" />
                    </el-select>
                     <el-button type="primary" @click="searchstudent" style="margin-left: 20px;">确认</el-button>
                    <el-select v-model="opname" placeholder="选择" style="width: 240px;margin-left: 200px;">
                        <el-option v-for="item in stunameData" :key="item.name" :label="item.name"
                            :value="item.name" />
                    </el-select>
                    <el-button type="primary" @click="showData" style="margin-left: 20px;">确认</el-button>
                </div>
            </div>
...
import { $getgrade } from '../../../api/studentdata/studentdata';
import {$getstudent} from '../../../api/studentdata/studentdata';
...
const opname = ref('')
const stunameData = ref([])

const searchgrade = reactive({
    gradeclass: ''
})
...
onMounted(() => {
    ...
    getstuall()
    getgrade()
    // console.log(opgrade.value);
    // opgrade.value =  searchgrade.gradeclass
    // getstuname()
});
...
const getstuname = async () =>{
    const res = await $getstudent(searchgrade)
    console.log(res);
    stunameData.value = res.data
    
}
const searchstudent = async () => {
    console.log(opgrade.value);
    searchgrade.gradeclass =   opgrade.value
    getstuname()
...
const showData = async() =>{
    ...
    searchForm.searchKeyword = opname.value
    getpapercount()
    getzlcount()
    gethjcount()
    getallpaper()
