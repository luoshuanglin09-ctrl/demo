  ...
  <div class="box">
    ...
    <el-dialog v-model="dialogVisible" title="发帖" width="500" :before-close="handleClose">
      ...
      <template #footer>
        <div class="dialog-footer">
          ...
          <el-button type="primary" @click="addposts">
            确认
          </el-button>
        </div>
      </template>
    ...
    <el-dialog v-model="dialogFormVisible" title="评论" width="500">
      ...
      <template #footer>
        <div class="dialog-footer">
          ...
          <el-button type="primary" @click="truecommit">
            确认
          </el-button>
        </div>
      </template>
    ...
    <el-dialog v-model="dialogTableVisible" title="评论区" width="60%">
      <div class="comments" v-for="item in commentsdata" :key="item.comment_id">
        ...
        <div class="comments-time">{{ formatDate(item.created_at) }}</div>
      </div>
    </el-dialog>
      ...
      <div class="post-space">
        <div class="post-content">
          ...
          <div class="post-content-bottom">
            ...
            <span style="margin-left: 20px; margin-right: 20px;">发布时间:{{ formatDate(post.CreatedTime) }}</span>
            <br />
            <el-button type="danger" link @click="delcommit(post.postId)">删除帖子</el-button>
    ...
    <div class="pagination">
      <el-pagination @size-change="handlePageSizeChange" @current-change="handleCurrentPageChange"
        ...
        layout="sizes, prev, pager, next, jumper" :total="posts.length">
      </el-pagination>
    </div>
  </div>
</template>
...
import { $addPosts, $delpots } from '../../../api/post/post'
import { $addcomments } from '../../../api/post/post'
import { $getcomments } from '../../../api/post/post'
...
//删除
const delcommit = async (postId) => {
  const confirmResult = await ElMessageBox.confirm('确定删除该帖子吗?', '提示', {
  ...
  if (confirmResult === 'confirm') {
    const res = await $delpots(postId)
    if (res.code == 200) {
      getpost()
      ElNotification({
        title: 'Success',
